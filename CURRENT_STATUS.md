...
- One shared look: title, a short left- or center-aligned intro, then the content on the first screen. Peach cards (`--color-cream`, now #ffdcb8; the token kept its old name) with a black border; black number badges; purple Averia labels; design tokens only; Ira Glass copy with unspaced em dashes.
...
- **Thinking Like an Artist** (`/thinking-like-an-artist/`; Related links to What's a Jerk?, Eight Ways to Look, and the Seven-Minute Look): a left-aligned intro for newer artists (draft B), then three pills (Wanting to be seen, Fear, Choices). Each tool's layers step light to dark (cream, gold, orange, crimson, black). Fear ends on a hopeful "What's actually true" layer (September 25, draft B).
...
- **The Seven-Minute Look** (`/seven-minute-look/`, replaces the Attention Lab; `/attention-lab/` redirects; in Artist Tools after Eight Ways to Look). A guided look done in person: a timer runs three looks (0:00–0:15 first glance, to 3:00 "Follow your eye": "Is there a path your eye keeps taking through the picture, or does it drift?" (Alan chose this wording so it's clearly about the eye, not walking around), to 7:00 keep finding things, with **Done looking** in the last one), dings and spoken cues (both can be turned off), and keeps the screen awake. Then six questions, each with "What did you see?" and a 0–5 score, and a running total out of 30. Nothing is saved. Timer approach, the two ding files (`public/sounds/`), and the iPhone audio unlock come from Alan's Life Drawing Timer web app (`~/Desktop/lifedrawingtimer-web`), ported to plain JavaScript. Every show page has "Standing in front of one? Try the Seven-Minute Look →" under On the wall. A quiet last line links to the Hidden Grammar of Art corpus (new tab). `src/data/attention-wheel-sample.json` is now unused.
- Movements, Artists in History, and the Art History Wheel weren't redesigned; Alan considers them already in line. The Art History Wheel's desktop layout was tightened (September 25): a two-line header ("19 movements, 1960s to now…"; "1960s to NOW" and "Eras / Styles" hidden on desktop only), a wheel sized to the screen (`--art-wheel-size`, 600px at most), and the panel showing the artwork first and centered, its title and artist, the Key Artists pills, the artwork description, then the movement description. At 1440×900 and 1280×760 the wheel and its buttons fit above the fold. Rules are appended at the end of `public/styles/art-wheel.css`; the phone layout is unchanged. `/artists/` was retitled "Artists in History" (heading and tab) to match the menu and stay distinct from Our Artists.
...
- Movements and Artists in History cards: thin black border and 12px corners like the rest of the site (drop shadows removed), movement colors unchanged. Movements uses the same grid as Artists in History (three across on a typical desktop). Card type warmed up: names and artist lists in Averia, descriptions regular weight instead of medium, the movement label on artist cards in small Averia capitals, and the text colors on tokens.
- Showing Now fix: an open-ended showing that hasn't opened yet reads "Opens Sep 28 · Ongoing" instead of the contradictory "Since Sep 28" (`shortDates` in `src/lib/showing-display.ts`).
