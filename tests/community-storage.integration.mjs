...
try{
 ...
 const venueApplication={kind:'venue',payload:{name:'Applicant Venue',type:'Gallery',regionId:'region-rogue-valley',city:'Ashland',address:'1 Test Street',postalCode:'97520',description:'Test gallery',website:'',phone:'',hours:'',accessibility:'',instructions:'',contactName:'Venue Owner',email:'venue@example.test',opportunities:'Wall space',available:true,agreeTerms:true,termsVersion:TERMS}};
 assert.equal((await call('applications','PUT',{...venueApplication,payload:{...venueApplication.payload,agreeTerms:false}},{},applicant)).status,400,'venues must agree to the terms too');
 ...
 const storedVenue=JSON.parse((await db.prepare("SELECT payload FROM community_records WHERE collection='venues' AND id=?1").bind(venueApplicationId).first()).payload);assert.equal(storedVenue.terms.version,TERMS,'the venue’s agreement is recorded');
