...
**Changes from v2:** (1) Displacement section replaced with seven named, specific suppression targets drawn from critics-and-frameworks.md v3 Frameworks to Displace; (2) STEP 0 ethical gate added as pre-analysis check; (3) Section A expanded with primary-source-backed quality criteria from Brody and Shafer; (4) Source coverage caveat added to routing; (5) Image analysis ceiling note added; (6) Mimbres sections now primary-source-supported — Hohokam, Ancestral Puebloan, Casas Grandes sections remain paraphrase-supported.

---

## How to use this block

This block goes into Pass 2 of an artifact analysis. Pass 1 runs first, without this block — it produces the universal perceptual inventory using the Hidden Grammar Principles. Pass 2 takes the Pass 1 output and routes it through this block to produce a tradition-calibrated evaluation.
...
Run these four checks before proceeding. They are gates, not criteria. An object that triggers a gate gets a different response than one that clears all four.
...
**Source coverage caveat:** Mimbres / Mogollon evaluative criteria in this block are primary-source-supported (Brody, Shafer — see source-extracts.md v3). Hohokam, Ancestral Puebloan, and Casas Grandes sections are paraphrase-supported pending primary source work. Adjust interpretive confidence accordingly when routing to non-Mimbres traditions.

---

**Identify the object class first. This determines which evaluative criteria apply.**
...
**Cultural tradition:**
[ ] Mimbres (Mogollon tradition, SW New Mexico, ~200–1150 CE) — primary-source-supported
[ ] Other Mogollon
[ ] Hohokam (S Arizona river valleys, ~300–1450 CE) — paraphrase-supported
[ ] Ancestral Puebloan (Four Corners region, ~750–1300 CE) — paraphrase-supported
[ ] Casas Grandes / Paquimé (NW Chihuahua, ~1200–1450 CE) — paraphrase-supported
[ ] Post-Classic Southwest (~1150–1450 CE, post-reorganization)
[ ] Other / unknown
...
Analyze this artifact using the evaluative framework of Southwest archaeology and anthropological artifact analysis. Select the criteria set that matches the identified object class. Apply all cross-cutting criteria (Section G) regardless of object class.
...
**Form and construction:** Describe rim diameter (estimate from proportions if not documented — state clearly when this is an estimate), depth, wall curvature, base form. Wall thickness consistency — even walls indicate skilled coil construction; variation signals production-stage issues. Evidence of coil-and-scrape: oblique striations, coil junctures at breaks or thin spots. Paddle-and-anvil finishing marks (Hohokam primary method vs. Mimbres coil-and-scrape). What does construction evidence say about skill and production tradition?
...
**Paint:** Mineral paint (matte or semi-matte, permanent, iron-based) vs. carbon/organic paint (potentially shiny surface, may flake). Read line quality directly — single-stroke control, weight consistency, evidence of hesitation or correction. Brush discipline is readable from the line work.
...
*Positive-negative ambiguity (Mimbres geometric only):* The Mimbres geometric system was explicitly built around the capacity of compositions to be read simultaneously as dark-on-light or light-on-dark. This is a specific technical achievement, not a general formal virtue. Assess: can this design be read in two ways simultaneously? If yes, this is a mark of the system's most successful geometric work.
...
**Firing and condition:** Firing atmosphere: oxidizing (tan/orange exterior) vs. reducing (gray/black). Fire clouds on exterior: present on most Mimbres bowls — this is expected and was deliberately tolerated, not an error. Fire clouds on interior: unusual, and their absence on interiors is deliberate (protective sherding in the kiln). Mixed black-and-red paint: may be deliberate aesthetic choice, not firing failure (Brody).

*Primary source note:* "It seems clear that Mimbres potters deliberately courted certain kinds of accidents and imperfections — an unexpected aspect of their aesthetic and a counterbalance to the perfection of patterning that they usually sought and often achieved in their paintings" (Brody).
...
**Preservation context:** Organic materials survive primarily in dry cave contexts. Preservation state (intact fiber, retained pigment, structural integrity) indicates intentional protected deposition vs. accidental burial vs. discard. Preservation quality is itself evidence about deposition.
...
*Mimbres obsidian sources:* Mule Creek (eastern Arizona), Ewe Canyon, Antelope Wells — all 120–150 km distant from the Mimbres Valley (Shafer). Obsidian presence = non-local exchange participation at minimum.
...
**Formal type:** Place in regional typological sequence. Mimbres Classic period arrow point types: Hinton (parallel side notches, concave base), Swarts (parallel side notches, convex base), Cosgrove (parallel side notches, multiple notches on one edge) — Shafer, NAN Ranch ch. 11. Ground stone metate sequence: oval basin → trough open one end → trough open both ends; correlates with maize intensification.
...
**Species identification:** Gulf of California species (Glycymeris, Conus, Olivella, Laevicardium) — primary Hohokam trade shell, ~300–500 km distance. Pacific coast species — longer-distance trade. Gulf of Mexico species (Spondylus, Busycon) — very long-distance trade, broad exchange networks. Spondylus presence is always analytically significant regardless of context. Species identification from photographs is approximate; note visible morphological features.

*NAN Ranch species counts for reference (Shafer):* Glycymeris 361, Nassarius 182, Pecten 137, Haliotis 81, Spondylus 74, Olivella 34, Conus 5, coral 2, Strombus 2. Spondylus at 74 specimens indicates sustained long-distance exchange participation.
...
**Social position of production:** Household production vs. part-time specialist vs. full-time specialist. Evidence: standardization (specialist production produces measurable regularity), production debris in context (workshops leave characteristic waste), material investment level. Full-time specialization is the exception in the prehistoric Southwest and requires specific evidence. For Mimbres: "no evidence suggests that anything approaching full-time craft specialization or professionalism existed" (Brody). Use "cottage-level craft specialist" (Shafer/Creel) for Mimbres production organization.
...
**Ancestral Puebloan ceramics (paraphrase-supported):** Pueblo Bonito material (Chaco Culture NHP, Smithsonian) for Chaco-affiliated contexts. Mesa Verde NP collections for 13th-century cliff dwelling assemblages. Maxwell Museum (UNM) for broad regional comparative material.
...
**v1 (May 2026):** First draft. Mimbres ceramics only. Four evaluative criteria sections. Not tested. Suspected drift risks: fine art vocabulary for high-quality figurative imagery; over-interpretation of iconographic content; conflation of Mimbres with Pueblo or Hohokam.
...
- `corpus/archaeological/critics-and-frameworks.md` — v3, Frameworks to Displace section added
- `corpus/archaeological/canonical-works.md` — v2, sites and object types
- `corpus/archaeological/evaluative-criteria.md` — v3, STEP 0 and Section H (image ceiling) added
- `corpus/archaeological/source-extracts.md` — v3, primary source extracts for Mimbres (Brody, Shafer); paraphrase for other traditions
- `corpus/displacement-library/archaeological-v1.md` — v1 (Mimbres ceramics only; retained as historical record)
- `corpus/displacement-library/archaeological-v2.md` — v2 (full Southwest object range; retained as historical record)
