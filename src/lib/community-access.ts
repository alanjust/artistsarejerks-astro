...
export async function requireCommunityAccess(context: Pick<APIContext, 'locals' | 'redirect'> & {response: {headers: Headers};url:URL}, role: 'administrator' | 'venue' | 'artist') {
  ...
  if (!access || (role === 'administrator' ? !access.administrator : role==='venue' ? !access.administrator&&(!access.venueId||context.url.searchParams.get('venue')!==access.venueId) : role==='artist' ? !access.administrator&&(!access.artistId||context.url.searchParams.get('artist')!==access.artistId) : true)) {
