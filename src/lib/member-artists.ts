...
export async function renderMemberDirectory(){await storageReady;const root=document.querySelector('.member-grid');if(!root)return;root.querySelectorAll('[data-member-profile]').forEach(n=>n.remove());for(const artist of readMemberArtists().filter(a=>a.published&&getMemberArtist(a.id))){const work=artist.works.find(isShown);if(!work)continue;const a=document.createElement('a');a.className='member-card';a.dataset.memberProfile=artist.id;a.href=memberUrl(artist.id);const image=document.createElement('img');image.alt=`Artwork by ${artist.name}`;image.src=await imageUrl(work);const copy=document.createElement('div');for(const [tag,text] of [['h2',artist.name],['p',artist.city],['p',artist.practice]]){const el=document.createElement(tag);el.textContent=text;copy.append(el)}if(work.madeWithAI){const label=document.createElement('span');label.className='aaj-ai-label';label.textContent='Made with AI';copy.append(label)}a.append(image,copy);root.append(a)}}
