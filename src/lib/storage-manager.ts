...
function snapshot(){const result:Record<string,string>={};for(const key of Object.values(COLLECTION_KEYS)){const value=localStorage.getItem(key);if(value)result[key]=value}return result}
...
export function initStorageManager(){
 ...
 for(const [collection,key] of Object.entries(COLLECTION_KEYS)){const rows=JSON.parse(localStorage.getItem(key)||'[]');const p=document.createElement('p');p.textContent=`${rows.length} ${collection}`;root.append(p)}
 ...
 async function connect(importData:boolean){buttons.forEach(b=>b.disabled=true);try{
  ...
  if(importData){
   ...
   for(const [collection,key] of Object.entries(COLLECTION_KEYS)){if(!current[key])continue;const payload=JSON.parse(current[key]);const rows=payload.map((row:Record<string,unknown>)=>({id:row.id,payload:row}));for(const row of rows)operations.push({collection,...row})}
