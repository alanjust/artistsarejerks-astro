...
function status(){let node=document.querySelector<HTMLElement>('[data-community-storage-status]');if(!node){node=document.createElement('div');node.dataset.communityStorageStatus='';node.setAttribute('role','status');node.style.cssText='padding:12px 24px;background:#000;color:#ffbf47;font-size:14px';document.body.prepend(node)}node.replaceChildren();node.append(document.createTextNode(error?`Shared storage: ${error}`:enabled?(pending.length?'Saving to shared storage…':(onlineTest?'Private test cloud storage · Saved':'Shared local storage · Saved')):'Browser-only prototype'));const link=document.createElement('a');link.href='/prototype/storage/';link.textContent=' · Storage settings';link.style.color='inherit';node.append(link)}
