...
const publicView=(loopback||onlineTest)&&!privatePreview&&(location.pathname==='/showing-now/'||location.pathname==='/our-artists/'||location.pathname==='/showing/'||location.pathname.startsWith('/prototype/artists/')||location.pathname==='/prototype/venues/'||location.pathname.startsWith('/prototype/venues/')&&!location.pathname.startsWith('/prototype/venues/workspace'));
...
function status(){let node=document.querySelector<HTMLElement>('[data-community-storage-status]');if(!node){node=document.createElement('div');node.dataset.communityStorageStatus='';node.setAttribute('role','status');node.style.cssText='padding:var(--space-3) var(--space-6);background:var(--color-black);color:var(--color-gold);font-size:var(--font-size-sm)';(document.querySelector('main.main-content')??document.body).prepend(node)}node.replaceChildren();node.append(document.createTextNode(error?`Shared storage: ${error}`:enabled?(pending.length?'Saving to shared storage…':(onlineTest?'Private test cloud storage · Saved':'Shared local storage · Saved')):'Browser-only prototype'));const link=document.createElement('a');link.href='/prototype/storage/';link.textContent=' · Storage settings';link.style.color='inherit';node.append(link)}
