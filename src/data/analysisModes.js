...
After the current-state report, add one final section:

## WHAT IF?
Identify the single most generative unresolved tension in the work as it currently stands — the
place where the visual evidence could go in genuinely different directions. Name the competing
principle readings without resolving them. Generate 2–3 experimental questions beginning with
"What if..." that follow from that tension. Do not suggest which direction to take. The unresolved
tension is productive territory, not a problem to fix.`,
      },
      {
        id: 'fine-art-surface',
...
Do not evaluate whether material agency is desirable. Describe what it produces.`,
  ...
  // ───────────────────────────────────────────
  // ANALYSIS TOOLS
  // Diagnostic reads — structural findings, dwell time, integrity checks.
  // Swaps the analytical approach; subject stays the same.
  // Phase 1 (Core) + Phase 2 (Studio Tools) modes from MODES_REGISTRY.
  // ───────────────────────────────────────────
  {
    id: 'analysis-tools',
    label: 'Analysis Tools',
    description: 'Diagnostic reads that return structural findings — where attention holds, where structure fails, what the work is physically doing. Switch the analytical approach without switching the subject.',
    subModes: [],
    fields: [
      { id: 'title',      label: 'Title' },
      { id: 'artist',     label: 'Artist' },
...
FORMAT: Three labeled sections as above. Prose within each section.`,
  ...
  // ───────────────────────────────────────────
  // HISTORIAN MODE
  // Academic art historical analysis built consonantally:
  // formal read first, lineage as competing resonances second,
  // open historical questions rather than placement verdict.
  // ───────────────────────────────────────────
  {
    id: 'historian',
    label: 'Historian Mode',
    description: 'Academic breakdown grounded in visual evidence. Locates the work within art history through what is formally present — not through received critical consensus. Historical positioning is held as competing resonances, not a single lineage verdict.',
    subModes: [],
    fields: [
      { id: 'artist',    label: 'Artist' },
      { id: 'title',     label: 'Title' },
      { id: 'period',    label: 'Period / Year' },
      { id: 'medium',    label: 'Medium' },
      { id: 'notes',     label: 'Context or research question (optional)' },
    ],
    prompts: [
      {
        id: 'historian-analysis',
        label: 'Historian Mode',
        description: 'Full academic analysis. Formal read first, principle mapping second, art historical resonances held as competing possibilities, open historical questions as the honest conclusion.',
        prompt: `You are conducting an art historical analysis using the Hidden Grammar framework. Your role is to locate this work within art history through the visual evidence alone — not through attribution, provenance, or received critical consensus.
...
- Resistance Gate: 1+ formal snag identified and explained
- Constraint Gate: 1+ conspicuous absence named
- Uncertainty Gate: 1+ genuine ambiguity held open
Check internally. Report as Coverage Gate: PASS/FAIL, Resistance Gate: PASS/FAIL, Constraint Gate: PASS/FAIL, Uncertainty Gate: PASS/FAIL before proceeding to Historical Resonances.

OUTPUT STRUCTURE — four sections in this order:

## FORMAL READ
Observable properties only, spatially located. No lineage claims, no period attributions, no artist comparisons in this section. Name the active Principles with confidence levels. Identify what is formally distinctive — what this work does that is uncommon. Identify what is conspicuously absent. This section is the consonantal base: visual evidence before interpretation.

## PRINCIPLE MAPPING
Map the active Principles to the 11 Roots. Identify which Roots are dominant, supporting, and silent. For each dominant Root, name the specific mechanism and its confidence level. Where the visual evidence is ambiguous between competing Principle readings, hold those ambiguities explicitly — do not resolve them.

## HISTORICAL RESONANCES
This is not a verdict on lineage — it is a mapping of resonances held as competing possibilities. For each formally distinctive observation from the Formal Read, name the art historical traditions or lineages whose visual language the evidence is consistent with. Use this structure:

"This [spatially located observation] is consistent with [tradition or lineage A] — and also with [tradition or lineage B]. The distinction matters historically because [one sentence on what formally separates the two lineages]."

Never assign the work to a single lineage. Never state "this is in the tradition of X" as a conclusion. The work may occupy more than one lineage simultaneously, sit at the intersection of two, or resist easy placement. Name all of these as possible positions.

## OPEN HISTORICAL QUESTIONS
Two or three questions that follow directly from the unresolved resonances above. These are not rhetorical — they are the genuine historical questions the visual evidence raises but cannot answer from observation alone. Frame each as:

"If this belongs to [lineage A], then [historical implication]. If [lineage B], then [different implication]. The visual evidence does not resolve this."

Do not produce a placement verdict. The open questions are the honest conclusion.`,
      },
    ],
  },
...
- Do not recommend fixes. Report the gap. The artist decides what to do with it.`,
  ...
  // For users prompting AI image generators.
  // Diagnostic mode: identifies what the image
  // is doing and what the next prompt should address.
  // ───────────────────────────────────────────
  {
    id: 'ai-generation',
    label: 'AI Generation Feedback',
    description: 'You generated this with Midjourney, DALL-E, Firefly, or another tool. Something feels off but you can\'t name it. This tells you what the image is actually doing perceptually — and what to address in the next prompt.',
    subModes: [],
    fields: [
    ...
    prompts: [
      {
        id: 'ai-gen-diagnostic',
        label: 'Generation Diagnostic',
        description: 'What is this image doing perceptually, what is collapsing, and what should the next prompt address?',
        prompt: `This image was generated by an AI image generator. The user is trying to close the gap between what they generated and what they intended. They are not an artist. They are a maker — an engineer, designer, developer, or creative technologist who works primarily with AI tools.

MODE OVERRIDE — the following base prompt rules are suspended for this mode only:
- "Never offer unsolicited advice or improvement suggestions" is suspended. Actionable next steps are the primary output of this mode.
- "Never make claims about what the work tries to do or fails to do" is suspended. Diagnostic language about what is collapsing perceptually is required.
All other base prompt rules remain fully in effect.

VOICE: Plain, direct, technical. Not art criticism. Not academic. Speak to someone who understands systems and wants to know what is broken and how to fix it. Skip the Ira Glass opening paragraph — replace it with a one-sentence diagnostic summary that names the dominant perceptual issue immediately.

OUTPUT STRUCTURE — three labeled sections, in this order:

## WHAT'S WORKING
Identify 2–3 perceptual mechanisms that are active and functioning. For each:
- Name the principle in plain language first, framework term in parentheses after.
- State what it is doing to the viewer's attention specifically.
- One sentence each. These are things the next prompt should preserve and reinforce.

## WHAT'S COLLAPSING
Identify 2–3 perceptual mechanisms that are absent, misfiring, or resolving tension they should be holding. For each:
- Name what is missing or broken in plain language.
- State the perceptual consequence: what does the viewer's nervous system do as a result?
- Be specific. "The background is flat" is not specific. "The background elements are at near-equal luminance values, which eliminates depth cue differentiation and collapses the figure-ground hierarchy into a single visual plane" is specific.
- One to two sentences each.

## NEXT PROMPT LANGUAGE
Translate the What's Collapsing findings into specific, copyable prompt language.
- Write 3–5 prompt fragments the user can add to or replace in their next generation attempt.
- Each fragment should be concrete and visual, not stylistic. Not "make it more dramatic." More like: "figure dominates frame with exterior contour weight heavier than interior line detail" or "background elements at significantly lower luminance than foreground, strong depth separation."
- Label each fragment with the perceptual principle it is targeting in parentheses.
- End with one sentence stating what to run through Art Lab next, and what to look for.
...
Do not use the term "Hidden Grammar" in output.`,
      ...
      {
        id: 'ai-gen-reference-compare',
        label: 'Compare to Reference',
        description: 'You have a target style or reference image in mind. This identifies the perceptual gap between what you generated and what you\'re after.',
        prompt: `This image was generated by an AI image generator. The user has a reference style or target in mind — either a named artist, a genre, or a specific visual quality they are trying to reproduce or approximate.

MODE OVERRIDE — the following base prompt rules are suspended for this mode only:
- "Never offer unsolicited advice or improvement suggestions" is suspended.
- "Never make claims about what the work tries to do or fails to do" is suspended.
- "Never imply intent on the part of the maker" is suspended for the reference analysis only — the reference artist's intentional decisions are fair to name.
All other base prompt rules remain fully in effect.

VOICE: Plain, diagnostic, specific. This is a gap analysis, not a critique.

If the user has named a reference style or artist in the notes or target field, begin by identifying 2–3 dominant perceptual mechanisms that define that reference style — what is it actually doing structurally that makes it recognizable. Use this as the baseline for comparison.

If no reference is named, analyze the generated image alone using the Generation Diagnostic prompt structure.

OUTPUT STRUCTURE — three labeled sections:

## WHAT THE REFERENCE IS DOING
Name 2–3 structural mechanisms that define the target style perceptually.
Plain language first, framework term in parentheses.
Be specific about what each mechanism produces in the viewer's nervous system.
This is the target the generation is measured against.

## WHERE THE GAP IS
For each mechanism identified above, state whether the generated image replicates it, approximates it, or misses it entirely.
Name what is present in the generated image instead.
State the perceptual consequence of the difference.
Do not use evaluative language. The gap is a structural observation, not a failure.

## NEXT PROMPT LANGUAGE
Translate each gap into specific, copyable prompt language targeting that mechanism.
Label each fragment with the mechanism it addresses.
3–5 fragments maximum.
End with one sentence stating what to run through Art Lab next and what gap to close first.
